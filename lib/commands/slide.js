...
function slide(src, dest, options, callback) {
    ...
    var js = (options.js || '').split(','),
        ...
        favicon = (options.favicon || slide._defaultFavicon),
    ...
    async.waterfall([
        function (callback) {
            async.waterfall([
                function (callback) {
                    slide._data(src, js, css, favicon, {
                        title: title,
                        textures: slide._texturesData(theme.textures)
                    }, callback);
                },
...
slide._texturesData = function (textures) {
    return Object.keys(textures).map(function (name) {
        var values = textures[name];
        return {
            name: name,
...
slide._data = function (src, js, css, favicon, data, callback) {
    async.waterfall([
        function (callback) {
            async.series([
                function (callback) {
                ...
                function (callback) {
                    async.waterfall([
                        function (callback) {
                            _base64img(favicon, callback);
                        },
                        function (data, callback) {
                            callback(null, {
                                data: data,
                                filename: path.basename(favicon)
                            });
                        }
                    ], callback);
                },
...
slide._html = function (html) {
    ...
    $('#' + id).children().toArray().map(function (elm) {
        ...
        $(elm).attr({
            'data-pr-fragment': true,
            'data-pr-velocity': isHeader ? '0.5' : '0.8'
        });
        var html = $.html(elm);
    ...
    var pageHtml = '<section class="pr-page"><div class="pr-page-background" data-pr-z="-100"' +
        ' data-pr-fragment="true"></div>%s</section>';
...
slide._defaultFavicon = path.resolve(basedir, 'doc/resources/parari-favicon.png');
/** Base css file. */
